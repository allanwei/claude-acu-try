...
export const CONFIG_PATH = process.env.ACUMATICA_CONFIG_PATH ??
    ...
    if (process.platform === 'win32' && fs.existsSync(CONFIG_PATH)) {
        try {
            ...
            try {
                ...
                execSync(`icacls "${CONFIG_PATH}" /inheritance:e /grant:r "${username}:(F)"`, { stdio: 'ignore' });
                fs.unlinkSync(CONFIG_PATH);
            }
            catch {
...
function setRestrictedPermissions(filePath) {
    if (process.platform === 'win32') {
        try {
            const username = os.userInfo().username;
            execSync(`icacls "${filePath}" /inheritance:r /grant:r "${username}:(R,W)"`, { stdio: 'ignore' });
        }
        catch {
