...
export function resolveEntity(type) {
    if (isFinanceType(type))
        return resolveFinanceEntity(type);
    if (isInventoryType(type))
        return resolveInventoryEntity(type);
    if (isCrmType(type))
        return resolveCrmEntity(type);
    throw new Error(`Unknown record type "${type}". Supported types: ${Object.keys(ALL_TYPES).join(', ')}`);
}
export function supportedActionsFor(type) {
